...
namespace nou
{
	...
	void SkeletalAnimClip::Update(float deltaTime, const Skeleton& skeleton)
	{
		...
		//update time on a frame by frame basis 
		m_timer += deltaTime;

		//check if the time is during the anim duration, if not, reset m_timer and positions and rotations 
		if (m_timer > m_anim.duration)
		{
			m_timer = 0;

			for (int i = 0; i < m_posFrame.size(); i++)
			{
				m_posFrame[i] = 0;
			}

			for (int i = 0; i < m_rotFrame.size(); i++)
			{
				m_rotFrame[i] = 0;
			}
